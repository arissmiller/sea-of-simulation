...
function createRandomPreset(baseState) {
  ...
  return {
    ...
    colorStyle: randomFrom(['neon-cyan', 'sunset-grid', 'aurora', 'magma', 'monochrome']),
    colorFunction: randomFrom(['pulse', 'bands', 'interference', 'swirl', 'drift']),
...
export class SeaOfSimulationProgram {
  ...
  init({ scene }) {
    this.scene = scene;
    this.scene.fog.color = new THREE.Color('#02030a');
    this.scene.fog.near = 18;
    this.scene.fog.far = 56;
